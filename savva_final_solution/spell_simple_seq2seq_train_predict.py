...
model_name = "spell_simple_seq2seq"
...
def buildSpellOHEModel(vocab_size):
    x_input = Input(shape=(None, vocab_size), name="input_ohe_seq")

    bi_rnn1 = Bidirectional(CuDNNLSTM(256, return_sequences=True), name="bi_rnn1")(x_input)
    ...
    skip_con = concatenate([x_input, bi_rnn1, bi_rnn2, bi_rnn3, bi_rnn4], name="skip_connection")
    ...
    return Model(inputs=x_input, outputs=[spell_pred, name_pred], name="spell_ohe_model")

K.clear_session()
model = buildSpellOHEModel(VOCAB_SIZE)
model.summary()
...
trn_seq = SimpleGenerator(trn_df.fullname.values, 
                          ...
                          ohe_input=True, ohe_output=True,
...
print("## Model evaluation and validation predicting")
