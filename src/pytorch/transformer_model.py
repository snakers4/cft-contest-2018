...
class TransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        if embeddings is None:
            assert (vocab and emb_size), 'Need vocab + emb_size or embeddings to continue'
        if (embeddings is not None) & (emb_size is not None) & (vocab is not None):
            warnings.warn('Using pretrained embeddings ignoring "emb_size" and "vocab"')
        if embeddings is not None:
            self.vocab = embeddings.size(0)
...
class MultipleChoiceHead(nn.Module):
    ...
    def __init__(self, clf_token, cfg):
        ...
        self.linear = nn.Linear(cfg.n_embd, cfg.clf_classes)
    ...
    def forward(self, h, x):
        ...
        clf_h = clf_h.view(-1, self.n_embd, 1)
